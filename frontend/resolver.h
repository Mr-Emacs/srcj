...
struct Resolver {
    ...
    Resolver() : current(std::make_unique<Scope>()) { register_intrinsics(); }
    void enter_scope() { current = std::make_unique<Scope>(std::move(current)); }
    void exit_scope()  { current = std::move(current->parent); }
    Symbol *lookup(const std::string &name) { return current->lookup(name); }
    void define(Symbol sym) { current->define(std::move(sym)); }
